...
export class MovementSystem {
    ...
    static checkForCollisions(currentEntity, nextPosition, allCollisionEntities) {
        ...
        const translationVector = new Vector2(nextPosition.x - (currentEntityGraphics.position.x + currentEntityCollision.offset), nextPosition.y - (currentEntityGraphics.position.y + currentEntityCollision.offset));
    ...
    static handlePlayerMovement(playerEntity) {
        ...
        if (playerControl.isJumping) {
            ...
            next = new Vector2(playerGraphics.position.x + playerMovement.velocity.x, playerGraphics.position.y - playerControl.jumpHeight);
        }
            ...
            next = new Vector2(playerGraphics.position.x + playerMovement.velocity.x, playerGraphics.position.y + (playerGravity.isEnabled ? playerGravity.weight : 0));
        }
            ...
            next = new Vector2(playerGraphics.position.x + playerMovement.velocity.x, playerGraphics.position.y);
        }

        if (next.x < 0) {
            next.x = GlobalConfig.GAME_WINDOW_WIDTH - playerGraphics.width;
        }
        else if ((next.x + playerGraphics.width) > GlobalConfig.GAME_WINDOW_WIDTH) {
            next.x = 0;
        }
    ...
    static tick() {
        ...
        for (const iEntity of MovementSystem.getMovingEntities()) {
            ...
            if (iEntity.hasComponent(PlayerControlledComponent.identifier)) {
                ...
                if (nextPosition.y >= GlobalConfig.GAME_WINDOW_HEIGHT) {
